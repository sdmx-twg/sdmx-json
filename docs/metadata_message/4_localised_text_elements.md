...
[IETF tools](https://tools.ietf.org/html/rfc4647#section-3.4).
...
Optionally, all available language matches according to the user’s preferred language
choices expressed through the HTTP content negotiation (Accept-Language header
parameter) can be provided for each localised text element.
...
elements and the knowledge about the locale) in contexts where all language
versions are required or where the information on the language used is required.
...
complete set of all available languages, then in the response message the
metadataSet property `isPartialLanguage` is to be set to `true`.**
